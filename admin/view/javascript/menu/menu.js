$( document ).ready(function() {
	...
	$('#layout-add-iframe').on('load', function(event) {
		...
		if (current_url.indexOf('design/menu/') < 0) {
			$('#layout-add').modal('hide');
			window.location.reload();
		} else if (current_url.indexOf('design/menu') > -1) {
	...
	$('input[name=\'filter_category_name\']').autocomplete({
		'source': function(request, response) {
			$.ajax({
				url: 'index.php?route=design/menu/autocomplete&token=' + token + '&filter_category_name=' +  encodeURIComponent(request),
				dataType: 'json',
	...
	$('input[name=\'filter_product_name\']').autocomplete({
		'source': function(request, response) {
			$.ajax({
				url: 'index.php?route=design/menu/autocomplete&token=' + token + '&filter_product_name=' +  encodeURIComponent(request),
				dataType: 'json',
				success: function(json) {
					response($.map(json, function(item) {
						return {
	...
	$('input[name=\'filter_manufacturer_name\']').autocomplete({
		'source': function(request, response) {
			$.ajax({
				url: 'index.php?route=design/menu/autocomplete&token=' + token + '&filter_manufacturer_name=' +  encodeURIComponent(request),
				dataType: 'json',
				success: function(json) {
					response($.map(json, function(item) {
						return {
	...
	$('input[name=\'filter_information_name\']').autocomplete({
		'source': function(request, response) {
			$.ajax({
				url: 'index.php?route=design/menu/autocomplete&token=' + token + '&filter_information_name=' +  encodeURIComponent(request),
				dataType: 'json',
				success: function(json) {
					response($.map(json, function(item) {
						return {
