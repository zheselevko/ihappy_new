...
class ControllerFeedSitemapPro extends Controller {
	...
	public function index() {
		...
		$data['action'] = $this->url->link('feed/sitemap_pro', 'token=' . $this->session->data['token'], 'SSL');
		...
		$data['data_feed'] = HTTP_CATALOG . 'index.php?route=feed/sitemap_pro';
		
		$data['yandex_feed'] = HTTP_CATALOG . 'index.php?route=feed/yandex_feed';

		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');

		$this->response->setOutput($this->load->view('feed/sitemap_pro.tpl', $data));
	}
